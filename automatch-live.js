...
function add(){if(!active)return;const matches=list();if(!matches.length)return;const p={...matches[cursor%matches.length],id:Date.now()+cursor};cursor++;found.push(p);while(found.length>10)found.shift();window.NexoraProspectData=found.slice();render();notify(p)}
