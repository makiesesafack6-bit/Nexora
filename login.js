...
document.getElementById('loginForm')?.addEventListener('submit',async e=>{e.preventDefault();clearError();const phone=document.getElementById('loginPhone').value.trim();const stored=await findStoredAccount(phone);if(!stored){setError('Aucun compte Nexora correspondant à ce numéro. Vérifiez le numéro ou créez un compte.');return;}try{const response=await fetch('/api/send-otp',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone,channel:selectedChannel})});const data=await response.json().catch(()=>({}));if(response.ok&&data.configured){sessionStorage.setItem('nexoraLoginRealOtp','1');sessionStorage.setItem('nexoraLoginPhone',phone);phoneLogin.classList.add('hidden');otpLogin.classList.remove('hidden');document.getElementById('loginOtpNote').textContent=`Un code ${selectedChannel==='whatsapp'?'WhatsApp':'SMS'} réel a été envoyé au ${phone}.`;document.getElementById('loginOtp').focus();return;}}catch(error){console.warn('[Nexora] real login OTP unavailable:',error);}const otp=String(Math.floor(100000+Math.random()*900000));sessionStorage.setItem('nexoraLoginOtp',otp);sessionStorage.removeItem('nexoraLoginRealOtp');sessionStorage.setItem('nexoraLoginPhone',phone);sessionStorage.setItem('nexoraLoginChannel',selectedChannel);phoneLogin.classList.add('hidden');otpLogin.classList.remove('hidden');document.getElementById('loginOtpNote').textContent=`Mode test local : aucun ${selectedChannel==='whatsapp'?'WhatsApp':'SMS'} réel n'est envoyé. Code de test : ${otp}.`;document.getElementById('loginOtp').focus();});

document.getElementById('loginOtpForm')?.addEventListener('submit',async e=>{e.preventDefault();const code=document.getElementById('loginOtp').value.trim();const phone=sessionStorage.getItem('nexoraLoginPhone')||'';let valid=false;if(sessionStorage.getItem('nexoraLoginRealOtp')==='1'){try{const response=await fetch('/api/check-otp',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone,code})});const data=await response.json().catch(()=>({}));valid=response.ok&&data.ok===true;}catch(error){valid=false;}}else valid=code===sessionStorage.getItem('nexoraLoginOtp');if(!valid){document.getElementById('loginOtp').setCustomValidity('Code incorrect ou expiré');document.getElementById('loginOtp').reportValidity();return;}document.getElementById('loginOtp').setCustomValidity('');const current=JSON.parse(localStorage.getItem('nexoraAccount')||'{}');localStorage.setItem('nexoraAccount',JSON.stringify({...current,signedIn:true,verifiedPhone:true}));localStorage.setItem('nexoraSession','active');sessionStorage.removeItem('nexoraLoginOtp');sessionStorage.removeItem('nexoraLoginRealOtp');sessionStorage.removeItem('nexoraLoginPhone');sessionStorage.removeItem('nexoraLoginChannel');startLoading();});
