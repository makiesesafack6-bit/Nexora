...
document.querySelector('.mini-button')?.addEventListener('click', (event) => { event.preventDefault(); window.location.href = 'onboarding.html'; });
document.querySelector('a[href="#login"]')?.addEventListener('click', (event) => { event.preventDefault(); window.location.href = 'onboarding.html'; });
...
const demoProfiles = [
  ['Kevin R.','Full-stack developer','Johannesburg','Kevin'], ['Grace T.','UX/UI designer','Lagos','Grace'], ['Maya S.','Social media strategist','Nairobi','Maya'], ['Jonathan P.','Video editor','Accra','Jonathan'], ['Esther W.','Business consultant','Kigali','Esther'], ['Noah A.','Graphic designer','Dakar','Noah']
];
const track = document.getElementById('testimonialTrack');
if (track) {
  demoProfiles.forEach(([name, role, city, seed]) => { const card = document.createElement('article'); card.className = 'testimonial-card'; card.innerHTML = `<img src="https://api.dicebear.com/9.x/personas/svg?seed=${seed}" alt="Avatar de démonstration ${name}" /><div><div class="stars">★★★★★</div><p>« Nexora m'aide à voir rapidement les opportunités qui correspondent à mon profil et à mes objectifs. »</p><strong>${name}</strong><span>${role} · ${city}</span></div>`; track.appendChild(card); });
  const originals = [...track.children]; originals.forEach((card) => track.appendChild(card.cloneNode(true))); let offset = 0;
  const step = () => { const first = track.children[0]; if (!first) return; const distance = first.getBoundingClientRect().width + 16; offset += distance; track.style.transition = 'transform .65s ease'; track.style.transform = `translateX(-${offset}px)`; if (offset >= distance * originals.length) window.setTimeout(() => { track.style.transition = 'none'; offset = 0; track.style.transform = 'translateX(0)'; }, 700); }; window.setInterval(step, 3200);
}
